...
const JURIS_SECOES = [
  { grupo: "Súmulas", itens: [
    { key: "sv",  nome: "Súmulas Vinculantes", link: "https://portal.stf.jus.br/jurisprudencia/sumariosumulas.asp?base=30" },
    { key: "ssf", nome: "Súmulas do STF", link: "https://portal.stf.jus.br/jurisprudencia/sumariosumulas.asp?base=26" },
    { key: "ssj", nome: "Súmulas do STJ", link: "https://scon.stj.jus.br/SCON/sumstj/" },
    { key: "tse", nome: "Súmulas do TSE", link: "https://www.tse.jus.br/jurisprudencia/sumulas" },
  ] },
  { grupo: "Teses", itens: [
    { key: "rg", nome: "Repercussão Geral (RG)", link: "https://portal.stf.jus.br/repercussaogeral/" },
    { key: "rr", nome: "Recurso Repetitivo (RR)", link: "https://processo.stj.jus.br/repetitivos/temas_repetitivos/" },
    { key: "jt", nome: "Jurisprudência em Teses (STJ)", link: "https://scon.stj.jus.br/SCON/jt/" },
  ] },
];
...
function Gaveta({ titulo, cnt, aberta, onToggle, children }) {
  return (
    <div className="juris-gav">
      <button className="juris-gav-head" onClick={onToggle}>
        <span className="chev" style={{ transform: aberta ? "rotate(90deg)" : "none" }}>▶</span>
        <span>{titulo}</span>
        {cnt != null && <span className="cnt">{cnt}</span>}
      </button>
      {aberta && <div className="juris-gav-body">{children}</div>}
    </div>
  );
}
// Página FONTES › Jurisprudência: Súmulas, Teses e Informativos (aulas do Mege).
function JurisFonte({ infoData, onToggle, onBack, feitos, total }) {
  const [aberto, setAberto] = useState({ informativos: true });
  const flip = (k) => setAberto((o) => ({ ...o, [k]: !o[k] }));
  return (
    <section className="editais-page es-page">
      <button className="edital-back" onClick={onBack}>← Voltar ao painel</button>
      <p className="eyebrow">FONTES</p>
      <h1 className="serif" style={{ marginBottom: 10 }}>Jurisprudência</h1>
      <p className="es-sub">Súmulas, teses e informativos — cada fonte abre no clique. Você acompanha os informativos pelas aulas do Mege.</p>

      {JURIS_SECOES.map((sec) => (
        <div key={sec.grupo}>
          <div className="juris-grupo">{sec.grupo}</div>
          {sec.itens.map((it) => (
            <Gaveta key={it.key} titulo={it.nome} aberta={!!aberto[it.key]} onToggle={() => flip(it.key)}>
              <div className="juris-vazio">
      ...
      <Gaveta titulo="Aulas do Mege (Informativos)" cnt={`${feitos}/${total} vistas`} aberta={!!aberto.informativos} onToggle={() => flip("informativos")}>
        {MEGE_AULAS.map((a) => (
          <div key={a.id} className="info-row juris-aula" onClick={() => onToggle("mege", a.id)}>
            <input type="checkbox" checked={!!infoData[`mege-${a.id}`]} onChange={() => {}} />
    ...
    <div className="root">
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        /* Jurisprudência: gavetas (abre/fecha) + chips das fontes */
        ...
        .juris-gav { border: 1px solid var(--line); border-radius: 13px; background: var(--surface-2); margin-bottom: 9px; overflow: hidden; }
        .juris-gav-head { display: flex; align-items: center; gap: 10px; width: 100%; text-align: left;
          font: inherit; font-size: 14px; font-weight: 600; color: var(--text); background: none; border: none;
          padding: 13px 15px; cursor: pointer; }
        .juris-gav-head:hover { background: var(--surface-3); }
        .juris-gav-head .chev { color: var(--muted); font-size: 12px; transition: transform .18s; }
        .juris-gav-head .cnt { margin-left: auto; font-size: 11.5px; color: var(--faint); font-weight: 500; }
        .juris-gav-body { padding: 6px 13px 15px; }
