...
function Constancia({ registros }) {
  ...
  for (let i = 27; i >= 0; i--) {
    ...
    const label = `${i === 0 ? "hoje, " : ""}${fmtDia(d)}${on ? " · estudou" : " · sem registro"}`;
    cells.push({ i, on, hoje: i === 0, label });
  }
  return (
    <div className="panel es-consta">
      ...
      <div className="consta-strip">
        {cells.map((c) => <span key={c.i} title={c.label} className={`consta-day${c.on ? " on" : ""}${c.hoje ? " today" : ""}`} />)}
      </div>
    </div>
  );
}
    ...
    <div className="root">
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .consta-day { aspect-ratio: 1; border-radius: 6px; background: var(--surface-2); border: 1px solid var(--line-2); cursor: help; }
