...
export default function App() {
  ...
  const simTotal = SIMULADOS.reduce((a, g) => a + g.provas.length, 0);
    ...
    <div className="root">
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .nav-editais.active .nav-dot, .sidenav .nav-editais:hover .nav-dot { background: var(--coral); box-shadow: 0 0 10px var(--coral); }
        .nav-editais.active { color: var(--coral); }
      ...
      <nav className="sidenav" aria-label="Navegação por seções">
        ...
        >
          ...
          <span className="nav-label">Editais verticalizados</span>
        </a>
        ...
        {NAV.map((n) => (
          <a
            key={n.id}
            href={`#${n.id}`}
            className={view === "main" && activeSec === n.id ? "active" : ""}
      ...
      <div className="wrap">
        ...
        <section id="sec-simulados" className="panel">
          ...
          {SIMULADOS.map((g) => {
            ...
            const yearDone = g.provas.filter((p) => (simData[p] || {}).checked).length;
            return (
              <div key={g.grupo} className="sim-year">
                ...
                {isYearOpen && (
                  <div className="year-body">
                    {g.provas.map((p) => {
                      const v = simData[p] || {};
                      const overall = provaOverall(p);
                      const isOpen = openProva === p;
                      return (
                        <div key={p} className="sim-prova">
                          <div className="sim-prova-row">
                            <input type="checkbox" checked={!!v.checked} onChange={() => toggleSim(p, { checked: !v.checked })} />
                            <button className="sim-name" onClick={() => setOpenProva(isOpen ? null : p)}>
                            ...
                            <div className="sim-mats">
                              {MATERIAS_SIM.map((m) => (
                                <label key={m.id} className="sim-mat">
                                  <span>{m.name}</span>
                                  <input className="score mono" placeholder="%" value={(v.mats && v.mats[m.id]) || ""} onChange={(e) => setSimMat(p, m.id, e.target.value)} />
                                </label>
                              ))}
                            </div>
                          )}
