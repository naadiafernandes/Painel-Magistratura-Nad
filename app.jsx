...
const JURIS_SECOES = [
  { grupo: "Súmulas", cor: "#E85D9A", txt: "#ffffff", itens: [
    { key: "sv",  curto: "Vinculantes", nome: "Súmulas Vinculantes", link: "https://portal.stf.jus.br/jurisprudencia/sumariosumulas.asp?base=30" },
    { key: "ssf", curto: "STF", nome: "Súmulas do STF", link: "https://portal.stf.jus.br/jurisprudencia/sumariosumulas.asp?base=26" },
    { key: "ssj", curto: "STJ", nome: "Súmulas do STJ", link: "https://scon.stj.jus.br/SCON/sumstj/" },
    { key: "tse", curto: "TSE", nome: "Súmulas do TSE", link: "https://www.tse.jus.br/jurisprudencia/sumulas" },
  ] },
  { grupo: "Teses", cor: "#7C4DE0", txt: "#ffffff", itens: [
    { key: "rg", curto: "RG", nome: "Repercussão Geral (RG)", link: "https://portal.stf.jus.br/repercussaogeral/" },
    { key: "rr", curto: "RR", nome: "Recurso Repetitivo (RR)", link: "https://processo.stj.jus.br/repetitivos/temas_repetitivos/" },
    { key: "jt", curto: "Juris em Teses", nome: "Jurisprudência em Teses (STJ)", link: "https://scon.stj.jus.br/SCON/jt/" },
  ] },
];
// Informativos (Mege): amarelo, com texto escuro pra ler bem.
const MEGE_COR = "#F4C430";
const MEGE_TXT = "#3d2f00";
...
const DISCURSIVAS_KEY = "tjsc-discursivas:v1";   // caderno de discursivas (fica na aba antes dos editais)
// normaliza o valor salvo de um tópico: aceita o formato antigo (booleano) e o novo (objeto de flags)
...
// guarda/recupera a seleção por posição de caractere dentro de um contêiner (sobrevive à re-renderização)
function leiSelOffsets(cont) {
  const sel = document.getSelection();
  if (!sel || !sel.rangeCount) return null;
  const range = sel.getRangeAt(0);
  if (!cont.contains(range.commonAncestorContainer)) return null;
  const pre = range.cloneRange(); pre.selectNodeContents(cont); pre.setEnd(range.startContainer, range.startOffset);
  const start = pre.toString().length;
  return { start, end: start + range.toString().length };
}
function leiSelRestore(cont, off) {
  ...
  while ((node = w.nextNode())) {
    ...
    if (!sN && pos + len >= off.start) { sN = node; sO = off.start - pos; }
    if (pos + len >= off.end) { eN = node; eO = off.end - pos; break; }
    pos += len;
  }
  if (sN && eN) { const r = document.createRange(); r.setStart(sN, sO); r.setEnd(eN, eO); const s = document.getSelection(); s.removeAllRanges(); s.addRange(r); }
}
...
// junta o antigo "Destaque" (título) + "Ver mais" (corpo) numa caixinha de texto só
function caixaCombina(n) {
  const corpo = (n.verMais && n.verMais.replace(/<[^>]*>/g, "").trim()) ? n.verMais : "";
  const tit = (n.destaque || "").trim();
  if (!tit) return corpo;
  return "<b>" + caixaEsc(tit) + "</b>" + (corpo ? "<br><br>" + corpo : "");
}
...
// caixinha de texto da Caixa de entrada — mesmo padrão das notas do edital: o conteúdo é
// escrito no DOM uma vez e o React não reescreve; e o texto-fantasma é uma CAMADA separada
// (não um ::before dentro do campo), pra o acento morto (~ ´ ` ^) compor normalmente.
function CaixaEdit({ html, plain, onSave, onFocus, className, placeholder }) {
  ...
  return (
    <div className="caixa-edwrap">
      <div ref={ref} className={(className || "") + " caixa-ed"} contentEditable suppressContentEditableWarning
        onFocus={onFocus}
        onBlur={() => onSave(plain ? ref.current.innerText : ref.current.innerHTML)} />
      <div className="caixa-ph" aria-hidden="true">{placeholder}</div>
    </div>
  );
}
...
function CaixaView({ onBack, onArquivarEdital }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      // funde título + corpo numa caixinha só (uma vez) — as notas viram texto puro
      arr = (arr || []).map((n) => (n && n.destaque && n.destaque.trim()) ? { ...n, verMais: caixaCombina(n), destaque: "" } : n);
      try { window.storage.set(CAIXA_KEY, JSON.stringify(arr)); } catch {}
  ...
  return (
    <section className="caixa-page">
      <style>{`
        ...
        .caixa-edwrap{ position:relative; }
        .caixa-ph{ position:absolute; left:0; top:0; color:var(--faint); pointer-events:none; }
        .caixa-ed:focus + .caixa-ph, .caixa-ed:not(:empty) + .caixa-ph{ display:none; }
        .caixa-destaque + .caixa-ph{ font-size:15px; font-weight:700; line-height:1.42; }
        .caixa-veditor + .caixa-ph{ top:11px; left:12px; font-size:15px; line-height:1.6; }
        .caixa-cardtop{ display:flex; align-items:center; gap:8px; margin-bottom:8px; min-height:20px; }
        ...
        .caixa-vtoolbar{ display:none; align-items:center; gap:6px; flex-wrap:wrap; padding:7px 9px; background:var(--surface-2); border:1px solid var(--line-2); border-radius:9px; margin-bottom:8px; }
        .caixa-card:focus-within .caixa-vtoolbar{ display:flex; }
        ...
        .caixa-veditor{ padding:11px 12px; font-size:15px; line-height:1.6; color:var(--text); min-height:60px; outline:none; border:1px solid var(--line-2); border-radius:11px; }
        .caixa-card:focus-within .caixa-veditor{ border-color:var(--gold); box-shadow:0 0 0 3px color-mix(in srgb,var(--gold) 14%,transparent); }
          ...
          {notas.map((n) => {
            return (
              <div key={n.id} className="caixa-card">
                ...
                <CaixaEdit className="caixa-veditor" html={caixaCombina(n)} placeholder="Escreva ou cole aqui…" onSave={(h) => update(n.id, { verMais: h, destaque: "" })} />
...
const TN_FORE = ["#e68a8a", "#7fa8e6", "#7bd39a"];   // começo enxuto — as recentes dela tomam conta
...
function NotaBox({ id, initialHtml, register, onBlur, onKeyDown, onInput, onMouseDown, onMouseMove, ph }) {
  const ref = useRef(null);
  useEffect(() => {
  ...
  // o texto-fantasma é uma CAMADA separada (não um ::before dentro do campo), pra
  ...
  return (
    <div className="tn-wrap">
      ...
      <div className="tn-ph" aria-hidden="true">{ph || "Escreva aqui…"}</div>
    </div>
  );
}

function TopicNotes({ label, materia, initial, backLabel, onBack, onSave, materias, resposta }) {
  const [boxes, setBoxes] = useState(() => {
    const arr = initial && initial.length ? initial : [{ id: TN_UID(), html: "", tags: [] }];
    return arr.map((b) => ({ id: b.id || TN_UID(), html: b.html || "", resp: b.resp || "", mat: b.mat || "", tags: (b.tags || []).map((t) => ({ label: t.label, ci: t.ci || 0 })) }));
  });
  const [filtroMat, setFiltroMat] = useState("");   // só usado quando a página tem matérias (caderno de discursivas)
  // respostas começam SEMPRE escondidas (a graça é tentar responder antes de olhar); não fica salvo
  ...
  const [recentes, setRecentes] = useState({ fore: TN_FORE, back: TN_BACK });
  const tagCounter = useRef(boxes.reduce((n, b) => n + b.tags.length, 0));

  useEffect(() => {
    (async () => {
      try { const r = await window.storage.get(TN_CORES_KEY); if (r) { const v = JSON.parse(r.value); setRecentes({ fore: v.fore || TN_FORE, back: v.back || TN_BACK }); } } catch {}
    })();
  }, []);
  // guarda a cor recém-usada no topo dos quadradinhos (sem repetir; "none" fica sempre no fundo)
  const guardarRecente = (kind, color) => {
    if (!color || color === "reset" || color === "transparent" || !/^#/.test(color)) return;
    setRecentes((prev) => {
      const atual = (prev[kind] || []).filter((c) => c !== "none");
      const lista = [color, ...atual.filter((c) => c.toLowerCase() !== color.toLowerCase())].slice(0, 7);
      const next = { ...prev, [kind]: kind === "back" ? ["none", ...lista] : lista };
      try { window.storage.set(TN_CORES_KEY, JSON.stringify(next)); } catch {}
      return next;
    });
  };
  ...
  useEffect(() => { try { document.execCommand("styleWithCSS", false, true); } catch (e) {} }, []);
  useEffect(() => {
    // NÃO ler a seleção enquanto o acento morto está compondo — ler a seleção durante a
    // composição faz o Chrome cortar a tecla morta (~ ´ etc.). Guarda a composição.
    let composing = false;
  ...
  const RID = (id) => id + "::resp";   // a caixinha da resposta guarda o conteúdo com esta chave
  const collect = () => boxes.map((b) => ({
    id: b.id,
    html: refs.current[b.id] ? refs.current[b.id].innerHTML : b.html,
    resp: refs.current[RID(b.id)] ? refs.current[RID(b.id)].innerHTML : (b.resp || ""),
    mat: b.mat || "",
    tags: b.tags,
  }));
  const persist = (arr) => onSave(arr.map((b) => ({ id: b.id, html: b.html, resp: b.resp || "", mat: b.mat || "", tags: b.tags })));
  ...
  // transforma a linha atual em título (T1/T2/T3) ou volta pra texto normal — vira um sumário dentro da caixinha
  const setTitulo = (tag) => { if (!tag) return; restore(); try { document.execCommand("formatBlock", false, tag); } catch (e) {} setTimeout(blurBox, 0); };
  const addTabela = () => {
    const cell = "<td><br></td>";
  ...
  const toggleResp = (id) => {
    const arr = collect();
    setBoxes(arr); persist(arr);
    setRespAberta((p) => ({ ...p, [id]: !p[id] }));
  };
  const setMat = (id, mat) => {
  ...
  return (
    <section className="tn-page">
      {onBack && <button className="edital-back" onClick={back}>{backLabel || "← Voltar ao edital"}</button>}
      ...
      <div className="tn-list">
        {boxes.filter((b) => !materias || !filtroMat || (filtroMat === "__sem" ? !b.mat : b.mat === filtroMat)).map((b) => (
          <div key={b.id} className={`tn-box${overId === b.id ? " over" : ""}`} onDragOver={(e) => onDragOver(e, b.id)} onDrop={(e) => onDrop(e, b.id)}>
            ...
            {resposta && (() => {
              const aberta = !!respAberta[b.id];
              const vazia = !String(b.resp || "").replace(/<[^>]*>/g, " ").replace(/&nbsp;/g, " ").trim();
              return (
                <div className="tn-resp-area">
                  <button type="button" className={`tn-resp-tog${aberta ? " on" : ""}${vazia ? "" : " tem"}`}
                    title={vazia ? "Abrir um espaço pra escrever a sua resposta" : "Mostrar a resposta que você escreveu"}
                    onClick={() => toggleResp(b.id)}>
                    {aberta ? "▾ esconder resposta" : vazia ? "▸ escrever resposta" : "▸ ver resposta"}
                  </button>
                  {aberta && (
                    <div className="tn-resp">
                      <div className="tn-resp-lbl">resposta</div>
                      <NotaBox id={RID(b.id)} initialHtml={b.resp} register={registerRef} onBlur={blurBox} onKeyDown={onKeyDownBox}
                        onInput={onInputBox} onMouseDown={onMouseDownBox} onMouseMove={onMouseMoveBox} ph="Escreva sua resposta…" />
                    </div>
                  )}
                </div>
              );
            })()}
            {(/<table/i.test(b.html) || (respAberta[b.id] && /<table/i.test(b.resp || ""))) && (
            ...
            <div className="tn-toolbar">
              ...
              <select className="tn-h" title="Título (vira um sumário na caixinha)" value="" onMouseDown={(e) => e.stopPropagation()} onChange={(e) => setTitulo(e.target.value)}>
...
// ===== CADERNO DA MATÉRIA: todas as anotações numa página só, na ordem do edital =====
// Junta as caixinhas de nota de cada tópico (e as observações do formato antigo) numa leitura
// corrida, pra rever a matéria inteira sem abrir tópico a tópico. Aqui é só leitura — pra
// escrever, ela clica em "editar" e cai na página de notas daquele tópico.
const cadVazia = (html) => !String(html || "").replace(/<[^>]*>/g, " ").replace(/&nbsp;/g, " ").trim();
function cadernoSecoes(materia, mi, dados) {
  const out = [];
  const walk = (node, key, depth, trilha) => {
    const label = `${node.n ? node.n + " " : ""}${node.txt}`.trim();
    const cell = editalCell((dados || {})[key]);
    let notas = (cell.notas || []).filter((b) => !cadVazia(b.html));
    if (!notas.length && cell.obs && cell.obs.trim()) {
      notas = [{ id: key + ":obs", html: cell.obs.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/\n/g, "<br>"), tags: [] }];
    }
    if (notas.length) out.push({ key, label, depth, trilha, notas: notas.map((b, i) => ({ id: b.id || key + ":" + i, html: b.html, tags: b.tags || [] })) });
    (node.subs || []).forEach((c, ci) => walk(c, `${key}.${ci}`, depth + 1, trilha.concat(label)));
  };
  (materia.itens || []).forEach((it, ii) => walk(it, `${mi}.${ii}`, 0, []));
  return out;
}

function CadernoMateria({ nome, secoes, editavel, onBack, onAbrirTopico }) {
  const total = secoes.reduce((n, s) => n + s.notas.length, 0);
  const irPara = (key) => {
    const el = document.getElementById("cad-" + key);
    if (el) el.scrollIntoView({ behavior: "smooth", block: "start" });
  };
  return (
    <section className="cad-page">
      <button className="edital-back" onClick={onBack}>← Voltar ao edital</button>
      <p className="eyebrow">Caderno da matéria</p>
      <h1 className="serif cad-title">{nome}</h1>
      {secoes.length === 0 ? (
        <p className="desc">Você ainda não tem anotações nesta matéria. Abra um tópico, escreva a sua nota — e ela aparece aqui, na ordem do edital.</p>
      ) : (
        <>
          <p className="cad-sub">
            {secoes.length} {secoes.length === 1 ? "tópico anotado" : "tópicos anotados"} · {total} {total === 1 ? "anotação" : "anotações"} — na ordem do edital.
          </p>
          <div className="cad-sumario">
            {secoes.map((s) => (
              <button key={s.key} className={`cad-sum-item d${Math.min(s.depth, 2)}`} onClick={() => irPara(s.key)}>{s.label}</button>
            ))}
          </div>
          {secoes.map((s) => (
            <article key={s.key} className="cad-sec" id={"cad-" + s.key}>
              {s.trilha.length > 0 && <div className="cad-trilha">{s.trilha.join(" › ")}</div>}
              <h2 className="cad-sec-tit">
                <span>{s.label}</span>
                {editavel && <button className="cad-edit" title="Abrir este tópico pra escrever" onClick={() => onAbrirTopico(s.key)}>✎ editar</button>}
              </h2>
              {s.notas.map((b) => (
                <div key={b.id} className="cad-box">
                  {b.tags.length > 0 && (
                    <div className="tn-tags">
                      {b.tags.map((t, i) => {
                        const pair = TN_TAGS[(t.ci || 0) % TN_TAGS.length];
                        return <span key={i} className="tn-tag" style={{ background: `rgba(${pair[1]},.16)`, color: pair[0] }}>{t.label}</span>;
                      })}
                    </div>
                  )}
                  <div className="tn-content" dangerouslySetInnerHTML={{ __html: b.html }} />
                </div>
              ))}
            </article>
          ))}
          <button className="cad-topo" onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })}>↑ voltar ao início</button>
        </>
      )}
    </section>
  );
...
function LeiSecaReader({ dip, onBack }) {
  ...
  const pintar = (cmd, valor) => {
    ...
    const off = leiSelOffsets(cont);                        // guarda a frase selecionada
    ...
    if (cmd === "uppercase") {
      // maiúsculas = formatação (não altera o texto por baixo; some com o × ou "limpar")
      try { const sp = document.createElement("span"); sp.style.textTransform = "uppercase"; sp.appendChild(s2.getRangeAt(0).extractContents()); s2.getRangeAt(0).insertNode(sp); } catch (e) {}
    } else if (cmd === "removeFormat") {
      try { document.execCommand("removeFormat", false, null); } catch (e) {}
      // removeFormat não tira maiúsculas/CSS — desembrulha os spans de estilo da seleção também
      try { const r = s2.getRangeAt(0); const frag = r.cloneContents(); if (frag.querySelector && frag.querySelector("span")) { const tmp = r.extractContents(); tmp.querySelectorAll && tmp.querySelectorAll("span[style]").forEach((s) => { while (s.firstChild) s.parentNode.insertBefore(s.firstChild, s); s.remove(); }); r.insertNode(tmp); } } catch (e) {}
    } else {
    ...
    // mantém a MESMA frase selecionada pra ela empilhar outra formatação sem re-selecionar
    setTimeout(() => { const c2 = [...document.querySelectorAll(".lei-txt")].find((el) => el.getAttribute("data-key") === key); if (c2) { leiSelRestore(c2, off); } }, 0);
  };
  ...
  // ⌘B/⌘U/⌘I pintam o TEXTO DA LEI selecionado sem precisar clicar na faixa (a caixinha de anotação cuida do seu próprio)
  useEffect(() => {
    const onKey = (e) => {
      ...
      if (n && n.closest && n.closest(".lei-nedit-in")) return;   // dentro da anotação: o editor cuida
      if (!n || !n.closest || !n.closest(".lei-txt")) return;
      e.preventDefault();
      pintar(m);
    };
  ...
  return (
    <section className="editais-page lei-reader">
      ...
      <div className="lei-paint">
        <button className="lei-paint-b" title="Negrito (⌘B)" style={{ fontWeight: 800 }} onMouseDown={pd} onClick={() => pintar("bold")}>B</button>
        ...
        <button className="lei-paint-b" title="Sublinhado (⌘U)" style={{ textDecoration: "underline" }} onMouseDown={pd} onClick={() => pintar("underline")}>U</button>
        <button className="lei-paint-b" title="Itálico (⌘I)" style={{ fontStyle: "italic" }} onMouseDown={pd} onClick={() => pintar("italic")}>I</button>
        <button className="lei-paint-b" title="Maiúsculas" style={{ fontSize: "11px", letterSpacing: ".5px" }} onMouseDown={pd} onClick={() => pintar("uppercase")}>AA</button>
        <span className="lei-paint-sep" />
        <span className="lei-paint-lbl">fonte</span>
        {LEI_CORES.map((c) => <button key={"f" + c} className="lei-paint-dot" style={{ background: c }} title="Cor da fonte" onMouseDown={pd} onClick={() => pintar("foreColor", c)} />)}
        <span className="lei-paint-sep" />
        <span className="lei-paint-lbl">fundo</span>
        {LEI_CORES.map((c) => <button key={"b" + c} className="lei-paint-dot" style={{ background: c }} title="Marca-texto (fundo)" onMouseDown={pd} onClick={() => pintar("hiliteColor", c)} />)}
        <span className="lei-paint-sep" />
        ...
        <span className="lei-paint-dica">selecione um trecho e pinte</span>
      </div>
...
function LeiLinha({ marca, texto, className, notaKey, nota, onSaveNota, marcas }) {
  ...
  // a caixinha nasce com a MESMA largura da unidade: recuo do inciso/alínea, largura cheia no caput/parágrafo
  const recuo = className === "lei-inc" ? 24 : className === "lei-alinea" ? 48 : 0;
  return (
    <div className="lei-linha">
      ...
      {(nota || anota) && (
        <LeiNota nota={nota} editing={anota} recuo={recuo}
...
function LeiNota({ nota, editing, onOpen, onClose, onSave, onDelete, recuo }) {
  const ref = useRef(null);
  ...
  if (!editing) {
    ...
    return (
      <div className="lei-nota" style={{ background: nota.cor, color: fg, marginLeft: recuo || 0 }}>
  ...
  return (
    <div className="lei-nedit" style={{ borderLeftColor: cor, marginLeft: recuo || 0 }}>
      ...
      <div className="lei-nedit-tb">
        <button title="Negrito (⌘B)" style={{ fontWeight: 800 }} onMouseDown={fmt("bold")}>B</button>
        <button title="Itálico (⌘I)" style={{ fontStyle: "italic" }} onMouseDown={fmt("italic")}>I</button>
        <button title="Sublinhado (⌘U)" style={{ textDecoration: "underline" }} onMouseDown={fmt("underline")}>U</button>
...
function JurisFonte({ infoData, onToggle, onBack, feitos, total }) {
  ...
  return (
    <section className="editais-page es-page">
      ...
      {/* Súmulas: moldura com as caixinhas (cor 1 = acento principal do tema) */}
      <div className="juris-secao-lbl">{JURIS_SECOES[0].grupo}</div>
      <div className="juris-panel">
        <div className="juris-cards">
          {JURIS_SECOES[0].itens.map((it) => (
            <button key={it.key} className="juris-card jcard-1" onClick={() => setSub(it.key)}>
      ...
      {/* Teses (cor 2) + Informativos (cor 3) na mesma moldura */}
      <div className="juris-panel-head">
        <span>{JURIS_SECOES[1].grupo}</span>
        <span>Informativos</span>
      </div>
      <div className="juris-panel">
        <div className="juris-cards">
          {JURIS_SECOES[1].itens.map((it) => (
            <button key={it.key} className="juris-card jcard-2" onClick={() => setSub(it.key)}>
              <span className="juris-card-nome">{it.nome}</span>
            </button>
          ))}
          <button className="juris-card jcard-3" onClick={() => setSub("mege")}>
...
function RegistroModal({ inicial, onClose, onSave }) {
  ...
  const [matFoco, setMatFoco] = useState(false);
  // sugestões de matéria ignorando acentos e maiúsculas (ç, ã, é... entram na busca)
  const matSug = (() => {
    const q = diaNorm(materia.trim());
    if (!q) return materias;
    return materias.filter((m) => diaNorm(m).includes(q) && diaNorm(m) !== q);
  })();
      ...
      <div className="reg-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="reg-body">
          ...
          <div className="reg-2col">
            <div className="reg-ac">
              <label className="reg-lbl">Matéria</label>
              <input className="reg-in" value={materia} autoComplete="off"
                onChange={(e) => setMateria(e.target.value)}
                onFocus={() => setMatFoco(true)}
                onBlur={() => setTimeout(() => setMatFoco(false), 130)}
                placeholder="ex.: Direito Administrativo" />
              {matFoco && matSug.length > 0 && (
                <div className="reg-ac-list">
                  {matSug.slice(0, 8).map((m) => (
                    <button type="button" key={m} className="reg-ac-item"
                      onMouseDown={() => { setMateria(m); setMatFoco(false); }}>{m}</button>
                  ))}
                </div>
              )}
            </div>
...
const QUESTOES_CADERNO_URL = "https://www.tecconcursos.com.br/questoes/cadernos/95973879";
            ...
            {d.topo ? (
              <div className="fila-topo" style={{ "--c": `var(${rosca.tok})` }}>
                ...
                <div className="fila-topo-acts">
                  ...
                  {rosca.id === "questoes" && <a className="fila-dod" href={QUESTOES_CADERNO_URL} target="_blank" rel="noopener noreferrer">📖 Abrir no TEC ↗</a>}
              ...
              {pend.slice(1).map((it) => (
                <li key={cicloItemKey(it)} className="fila-row">
                  ...
                  {rosca.id === "juris"
                    ...
                    : rosca.id === "questoes"
                    ? <a className="fila-nm fila-nm-link" href={QUESTOES_CADERNO_URL} target="_blank" rel="noopener noreferrer" title="Abrir o caderno no TEC">{it.nome} <span className="fila-nm-ext">↗</span></a>
...
// nota livre da Nadia num item de revisão: "o que revisar / de onde continuar"
function RevLembrete({ nota, onSave }) {
  const [editando, setEditando] = useState(false);
  const [txt, setTxt] = useState(nota || "");
  useEffect(() => { if (!editando) setTxt(nota || ""); }, [nota, editando]);
  const salvar = () => { onSave(txt.trim()); setEditando(false); };
  const cancelar = () => { setTxt(nota || ""); setEditando(false); };
  if (editando) {
    return (
      <div className="rev-lembrete-edit">
        <textarea className="rev-lembrete-in" value={txt} autoFocus rows={2}
          onChange={(e) => setTxt(e.target.value)}
          onKeyDown={(e) => { if (e.key === "Enter" && (e.metaKey || e.ctrlKey)) salvar(); if (e.key === "Escape") cancelar(); }}
          placeholder="o que revisar / de onde continuar — ex.: Refazer meu caderno de erros do TEC" />
        <div className="rev-lembrete-acts">
          <button className="rev-btn ok" onClick={salvar}>salvar</button>
          <button className="rev-btn" onClick={cancelar}>cancelar</button>
          {nota && <button className="rev-btn" onClick={() => { onSave(""); setEditando(false); }}>apagar</button>}
        </div>
      </div>
    );
  }
  if (nota) {
    return (
      <button className="rev-lembrete" onClick={() => setEditando(true)} title="Editar lembrete">
        <span className="rev-lembrete-ico">🔖</span><span>{nota}</span>
      </button>
    );
  }
  return <button className="rev-lembrete-add" onClick={() => setEditando(true)}>+ lembrete de revisão</button>;
}

// ===== ESTUDEI: Revisões (só o que você concluiu; sem prazo) =====
function RevisoesView({ registros, revStatus, onMark, onEdit, onSaveNota, onBack }) {
        ...
        return (
          <div className="rev-item" key={r.id}>
            ...
            <div className="rev-actions">
              <button className="hist-edit" onClick={() => onEdit(r)} title="Editar registro">✎</button>
            ...
            <RevLembrete nota={r.revNota} onSave={(txt) => onSaveNota(r.id, txt)} />
          </div>
        );
      })}
    </section>
...
export default function App() {
  ...
  const [openCaderno, setOpenCaderno] = useState(null); // { editalId, mi } — caderno com a matéria inteira anotada
  const [discNotas, setDiscNotas] = useState([]);       // caixinhas do Caderno de Discursivas
  ...
  const [navOpen, setNavOpen] = useState(() => {
    ...
    if (window.innerWidth < 1025) return false;            // celular/tablet: começa recolhido (gaveta)
    try { return localStorage.getItem("navOpen") !== "0"; } catch (e) { return true; }
  });
  ...
  useEffect(() => {
    (async () => {
      ...
      let disc = [];
      try { const r = await window.storage.get(DISCURSIVAS_KEY); disc = r ? JSON.parse(r.value) : []; } catch { disc = []; }
      let prio = [];
      ...
      setCursoData(cursos);
      setEditaisData(eds);
      setDiscNotas(Array.isArray(disc) ? disc : []);
      setDiscLoaded(true);
      setPrioridades(prio);
      setEsCheck(esck);
      setRegistros(regs);
      ...
      if (nav.openCaderno) setOpenCaderno(nav.openCaderno);
      if (nav.matAberta) setMatAberta(nav.matAberta);
      setLoaded(true);
    })();
  }, []);

  const saveDiscursivas = (notas) => {
    setDiscNotas(notas);
    try { window.storage.set(DISCURSIVAS_KEY, JSON.stringify(notas)); } catch {}
  };
  ...
  const saveRevNota = (id, texto) => saveRegistros(registros.map((r) => (r.id === id ? { ...r, revNota: texto } : r)));
  ...
  useEffect(() => {
    if (!loaded) return;
    try { localStorage.setItem(NAV_KEY, JSON.stringify({ view, openTopic, openCaderno, matAberta })); } catch {}
  }, [loaded, view, openTopic, openCaderno, matAberta]);
  ...
  return (
    <div className={`root${navOpen ? "" : " nav-hidden"}`}>
      <style>{`
        :root {
        ...
        :root[data-cor="amarelo"] { --gold:#EAA92E; --accent-btn:#EAA92E; --gold2:#E07A54; --coral:#2B5A82; --on-accent:#3a2c08; --js1t:#241a02; --js2t:#241a02; --js3t:#ffffff; }
        :root[data-cor="rosa"] { --gold:#E85D9A; --accent-btn:#E85D9A; --gold2:#F2B23A; --coral:#2F6FE4; --on-accent:#ffffff; --js1t:#ffffff; --js2t:#241a02; --js3t:#ffffff; }
        :root[data-cor="azul"] { --gold:#2F6FE4; --accent-btn:#2F6FE4; --gold2:#EE3B34; --coral:#F2B23A; --on-accent:#ffffff; --js1t:#ffffff; --js2t:#ffffff; --js3t:#241a02; }
        :root[data-cor="roxo"] { --gold:#7C4DE0; --accent-btn:#7C4DE0; --gold2:#F2B23A; --coral:#17B8A6; --on-accent:#ffffff; --js1t:#ffffff; --js2t:#241a02; --js3t:#241a02; }
        * { box-sizing: border-box; }
        ...
        @media (min-width: 1025px) {
          ...
          .root.nav-hidden { padding-left: clamp(16px,3vw,40px); }
        }
        ...
        .side-collapse { margin-left: auto; width: 30px; height: 30px; border-radius: 9px; display: grid; place-items: center;
          border: 1px solid var(--line-2); background: var(--surface); color: var(--muted); cursor: pointer; flex-shrink: 0; }
        ...
        /* botão flutuante pra reabrir o menu (aparece quando ele está recolhido) */
        .nav-toggle { position: fixed; top: 53px; left: 12px; z-index: 400; display: grid;
        ...
        /* Jurisprudência: molduras por grupo + caixinhas coloridas + subpágina + anotações */
        ...
        .juris-secao-lbl { font-size: 16px; font-weight: 700; color: var(--text); margin: 6px 0 8px; padding-left: 16px; }
        .juris-panel-head { display: flex; justify-content: space-between; align-items: baseline; max-width: 792px; margin: 20px 0 8px; padding: 0 16px; }
        .juris-panel-head span { font-size: 16px; font-weight: 700; color: var(--text); }
        .juris-panel { max-width: 792px; border: 1px solid var(--line); border-radius: 18px; padding: 16px; background: var(--surface); }
        .juris-cards { display: grid; grid-template-columns: repeat(4, 1fr); gap: 14px; }
        ...
        /* a aba do caderno de discursivas não é edital nenhum — fica marcada e um pouco separada */
        .edital-tab.disc { border-style: dashed; border-color: var(--gold); color: var(--gold); margin-right: 6px; }
        .edital-tab.disc:hover { color: var(--gold); border-color: var(--gold); }
        .edital-tab.disc.active { background: var(--gold); color: var(--on-accent); border-style: solid; }
        ...
        /* botão CADERNO (topo da matéria) + página do caderno da matéria */
        .ed-caderno-row { display: flex; align-items: center; gap: 10px; flex-wrap: wrap; padding: 9px 16px; border-top: 1px solid var(--line); background: var(--surface); }
        .ed-caderno { display: inline-flex; align-items: center; gap: 7px; cursor: pointer; font: inherit; font-size: 11px; font-weight: 800;
          letter-spacing: .09em; text-transform: uppercase; color: var(--gold); background: rgba(240,200,90,.10);
          border: 1px solid rgba(240,200,90,.42); border-radius: 999px; padding: 5px 14px; transition: all .14s; }
        .ed-caderno:hover { background: var(--gold); border-color: var(--gold); color: var(--on-accent); }
        .ed-caderno-ic { font-size: 12px; line-height: 1; }
        .ed-caderno-n { font-size: 10.5px; font-weight: 700; letter-spacing: 0; background: var(--green-bg); color: var(--green);
          border-radius: 99px; padding: 1px 7px; }
        .ed-caderno:hover .ed-caderno-n { background: rgba(0,0,0,.14); color: inherit; }
        .ed-caderno-dica { font-size: 11.5px; color: var(--faint); }
        .cad-page { max-width: 860px; }
        .cad-title { margin: 2px 0 4px; }
        .cad-sub { color: var(--muted); font-size: 13px; margin: 0 0 18px; }
        .cad-sumario { display: flex; flex-direction: column; align-items: flex-start; gap: 2px; margin: 0 0 26px;
          padding: 12px 14px; background: var(--surface-2); border: 1px solid var(--line-2); border-radius: 14px; }
        .cad-sum-item { border: none; background: transparent; cursor: pointer; font: inherit; font-size: 12.5px; color: var(--muted);
          text-align: left; padding: 2px 0; line-height: 1.4; }
        .cad-sum-item:hover { color: var(--gold); text-decoration: underline; text-underline-offset: 2px; }
        .cad-sum-item.d1 { padding-left: 16px; } .cad-sum-item.d2 { padding-left: 32px; }
        .cad-sec { margin: 0 0 30px; scroll-margin-top: 18px; }
        .cad-trilha { font-size: 11px; color: var(--faint); margin-bottom: 3px; }
        .cad-sec-tit { display: flex; align-items: baseline; gap: 10px; margin: 0 0 10px; font-size: 15.5px; font-weight: 800;
          color: var(--coral); line-height: 1.35; padding-bottom: 6px; border-bottom: 1px solid var(--line-2); }
        .cad-edit { margin-left: auto; flex-shrink: 0; border: none; background: transparent; cursor: pointer; font: inherit;
          font-size: 11.5px; font-weight: 600; color: var(--faint); }
        .cad-edit:hover { color: var(--gold); }
        .cad-box { background: var(--surface); border: 1px solid var(--line); border-radius: 16px; padding: 14px 16px; margin: 10px 0; }
        .cad-topo { display: block; margin: 6px auto 40px; border: 1px solid var(--line-2); background: var(--surface-2); color: var(--muted);
          border-radius: 999px; padding: 6px 16px; font: inherit; font-size: 12px; cursor: pointer; }
        .cad-topo:hover { color: var(--gold); border-color: var(--gold); }
        ...
        /* resposta escondida embaixo de cada questão (caderno de discursivas) */
        .tn-resp-area { margin-top: 10px; }
        .tn-resp-tog { font: inherit; font-size: 11.5px; font-weight: 700; cursor: pointer; color: var(--faint);
          background: transparent; border: 1px dashed var(--line-2); border-radius: 999px; padding: 3px 11px; letter-spacing: .03em; }
        .tn-resp-tog:hover { color: var(--gold); border-color: var(--gold); }
        .tn-resp-tog.tem { color: var(--gold); border-color: color-mix(in srgb, var(--gold) 45%, transparent); }
        .tn-resp-tog.on { border-style: solid; color: var(--muted); }
        .tn-resp { margin-top: 9px; padding: 10px 13px; background: var(--surface-2); border-left: 3px solid var(--gold); border-radius: 0 10px 10px 0; }
        .tn-resp-lbl { font-size: 10px; font-weight: 800; letter-spacing: .1em; text-transform: uppercase; color: var(--faint); margin-bottom: 5px; }
        ...
        .tn-ph { position: absolute; top: 0; left: 0; font-size: 15px; line-height: 1.62; color: var(--faint); pointer-events: none; }
        ...
        .tn-content h1 { font-size: 18px; font-weight: 800; color: var(--gold); margin: 12px 0 6px; padding-bottom: 3px; border-bottom: 1px solid color-mix(in srgb, var(--gold) 30%, transparent); }
        .tn-content h2 { font-size: 15.5px; font-weight: 800; color: var(--text); margin: 10px 0 5px; }
        ...
        @media (max-width: 720px) {
          ...
          .ed-caderno-row { padding-left: 10px; padding-right: 10px; }
          .ed-caderno-dica { display: none; }
        }
        ...
        .reg-ac { position: relative; }
        .reg-ac-list { position: absolute; left: 0; right: 0; top: 100%; margin-top: 4px; z-index: 20;
          background: var(--surface); border: 1px solid var(--line-2); border-radius: 10px; padding: 4px;
          box-shadow: 0 12px 30px rgba(0,0,0,.28); max-height: 232px; overflow-y: auto; }
        .reg-ac-item { display: block; width: 100%; text-align: left; background: none; border: none; cursor: pointer;
          font: inherit; font-size: 13.5px; color: var(--text); padding: 8px 10px; border-radius: 7px; }
        .reg-ac-item:hover { background: var(--surface-3); }
        ...
        .rev-item { display: flex; align-items: center; gap: 12px; padding: 12px 4px; border-bottom: 1px solid var(--line); flex-wrap: wrap; }
        .rev-when { font-size: 12px; color: var(--faint); margin-top: 2px; }
        .rev-actions { display: flex; align-items: center; gap: 8px; flex: none; }
        .rev-lembrete, .rev-lembrete-add, .rev-lembrete-edit { width: 100%; order: 9; }
        .rev-lembrete-add { background: none; border: 1px dashed var(--line-2); color: var(--faint); border-radius: 8px; padding: 7px 11px; font: inherit; font-size: 12px; font-weight: 600; cursor: pointer; text-align: left; }
        .rev-lembrete-add:hover { color: var(--muted); border-color: var(--muted); }
        .rev-lembrete { display: flex; align-items: flex-start; gap: 7px; background: var(--surface-2); border: 1px solid var(--line-2); border-left: 3px solid var(--gold); border-radius: 8px; padding: 8px 11px; font: inherit; font-size: 12.5px; color: var(--text); cursor: pointer; text-align: left; line-height: 1.45; white-space: pre-wrap; }
        .rev-lembrete:hover { border-color: var(--gold); }
        .rev-lembrete-ico { flex: none; }
        .rev-lembrete-in { width: 100%; box-sizing: border-box; background: var(--surface-2); border: 1px solid var(--line-2); border-radius: 8px; padding: 8px 11px; font: inherit; font-size: 12.5px; color: var(--text); line-height: 1.45; resize: vertical; }
        .rev-lembrete-in:focus { outline: none; border-color: var(--gold); }
        .rev-lembrete-in::placeholder { color: var(--faint); }
        .rev-lembrete-acts { display: flex; gap: 8px; margin-top: 6px; }
        ...
        .lei-paint { position: sticky; top: 56px; z-index: 20; display: flex; align-items: center; gap: 6px; flex-wrap: wrap; margin: 16px 0 4px; padding: 8px 10px; background: var(--surface); border: 1px solid var(--line); border-radius: 12px; box-shadow: 0 6px 20px rgba(0,0,0,.35); }
        ...
        .lei-paint-lbl { font-size: 10px; font-weight: 700; color: var(--faint); text-transform: uppercase; letter-spacing: .04em; }
        ...
        .lei-leg { color: var(--faint); font-size: .86em; }
        .lei-linha { margin: 0; }
        ...
        .lei-nota { display: flex; align-items: flex-start; gap: 8px; border-radius: 10px; padding: 9px 12px; margin: 8px 0; box-shadow: 0 2px 8px rgba(0,0,0,.18); }
        ...
        .lei-nedit { background: var(--surface); border: 1px solid var(--line); border-left: 3px solid var(--gold); border-radius: 12px; margin: 10px 0 0; overflow: hidden; }
      ...
      <aside className={`sidebar${navOpen ? "" : " closed"}`} aria-label="Navegação">
        <div className="side-logo">
          ...
          <button className="side-collapse" onClick={() => setNavOpen(false)} title="Recolher menu" aria-label="Recolher menu">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              <line x1="4" y1="7" x2="20" y2="7" /><line x1="4" y1="12" x2="20" y2="12" /><line x1="4" y1="17" x2="20" y2="17" />
            </svg>
          </button>
          ...
          <div className="nav-group" key={g.label}>
            ...
            {g.items.map((it) => {
              ...
              const handle = (e) => {
                ...
                setOpenCaderno(null); // idem pro caderno da matéria
      ...
      <div className="wrap">
        {openTopic && (() => {
          ...
          return (
            <TopicNotes
              ...
              initial={initial}
              backLabel={openCaderno ? "← Voltar ao caderno" : "← Voltar ao edital"}
              onBack={() => setOpenTopic(null)}
              onSave={(notas) => saveEditalNotes(openTopic.editalId, openTopic.key, notas)}
            />
          );
        })()}
        {view === "editais" && !openTopic && openCaderno && (() => {
          const ed = EDITAIS.find((e) => e.id === openCaderno.editalId);
          const m = ed && ed.materias[openCaderno.mi];
          if (!m) return null;
          return (
            <CadernoMateria
              key={ed.id + ":" + openCaderno.mi}
              nome={m.nome}
              secoes={cadernoSecoes(m, openCaderno.mi, editaisData[ed.id] || {})}
              editavel={ed.id === "tjsc-juiz-2025"}
              onBack={() => { setOpenCaderno(null); window.scrollTo(0, 0); }}
              onAbrirTopico={(key) => {
                const path = String(key).split(".").slice(1).map(Number);
                let node = m.itens[path[0]];
                for (let i = 1; i < path.length && node; i++) node = (node.subs || [])[path[i]];
                if (node) openTopicPage(ed, key, node);
              }}
            />
          );
        })()}
        {view === "editais" && !openTopic && !openCaderno && (
          <section className="editais-page">
            ...
            <div className="edital-tabs">
              {/* antes dos editais: o caderno de discursivas (não é prova nenhuma, é dela) */}
              <button className={`edital-tab disc${openEdital === -1 ? " active" : ""}`} onClick={() => { setOpenEdital(-1); window.scrollTo(0, 0); }}>
                ✎ CADERNO DE DISCURSIVAS
              </button>
              {EDITAIS.map((e, i) => (
                  ...
                  const renderNode = (node, key, depth) => {
                    ...
                    return (
                      <React.Fragment key={key}>
                        <div className={`ed-row d${depth}${RICH && hasNotas ? " has-notas" : ""}`}>
                    ...
                    <div className="edital-table">
                      ...
                      {matOrder.map(({ m, mi }, idx) => {
                        ...
                        const countNode = (node) => {
                          total++;
                          (node.subs || []).forEach((c) => countNode(c));
                        };
                        m.itens.forEach((it) => countNode(it));
                          ...
                          <React.Fragment key={mi}>
                            ...
                            <div className="ed-materia">
                              <button className={`ed-band${matOpen ? " open" : ""}`} onClick={() => setOpenMat((p) => ({ ...p, [matKey]: !p[matKey] }))}>
                                ...
                                <h3>{m.nome}</h3>
                                <span className="mono edital-count">{total}</span>
                              </button>
                              {matOpen && (() => {
                                // quantas anotações esta matéria já tem (pra mostrar no botão)
                                const nNotas = cadernoSecoes(m, mi, v).reduce((n, sec) => n + sec.notas.length, 0);
                                return (
                                  <div className="ed-caderno-row">
                                    <button
                                      type="button"
                                      className={`ed-caderno${nNotas ? " has" : ""}`}
                                      title="Ler todas as suas anotações desta matéria de uma vez, na ordem do edital"
                                      onClick={() => { setOpenCaderno({ editalId: ed.id, mi }); window.scrollTo(0, 0); }}
                                    >
                                      <span className="ed-caderno-ic">▤</span>
                                      CADERNO
                                      {nNotas > 0 && <span className="ed-caderno-n">{nNotas}</span>}
                                    </button>
                                    <span className="ed-caderno-dica">
                                      {nNotas ? "tudo o que você anotou nesta matéria, na ordem do edital" : "quando você anotar nos tópicos, tudo aparece aqui junto"}
                                    </span>
                                  </div>
                                );
                              })()}
        ...
        {view === "es-revisoes" && (
          <RevisoesView registros={registros} revStatus={revStatus} onMark={marcarRevisao} onEdit={abrirRegistro} onSaveNota={saveRevNota} onBack={() => setView("es-painel")} />
        )}
